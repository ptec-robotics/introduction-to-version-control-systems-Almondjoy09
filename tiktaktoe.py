...
def o_turn(board, turn):
  for i in range(3):
    row = get_row(board, i)
    if count(row, O) == 2 and count(row, EMPTY) == 1:
        if is_empty(board, i, 0): put (board, i, 0, O)
        if is_empty(board, i, 1): put (board, i, 1, O)
        if is_empty(board, i, 2): put (board, i, 2, O)
        return # play in the empty space
    
  for i in range(3):
    col = get_col(board, )
    if count(col, O) == 2 and count(col, EMPTY) == 1:
        return # play in emtpy space
  
  d1 = get_d1(board)
  if count(d1, O) == 2 and count(d1, EMPTY) == 1:
    return # play in empty space
  
  d2 = get_d2(board)
  if count(d2, O) == 2 and count(d2, EMPTY) == 1:
    return # play in empty space
